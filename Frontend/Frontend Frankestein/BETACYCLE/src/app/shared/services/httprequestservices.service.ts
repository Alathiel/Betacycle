...
export class HttprequestservicesService {
  ...
  //DATI UTENTE CONNESO
  GetUserInfo(id:number,token:any):Observable<any>
  {
    var head=this.authJwtHeader.set(
      'Authorization',
      'Bearer '+token
    )
  ...
  //Address
  GetAddresses(id:number,token:any)
  {
    var head=this.authJwtHeader.set(
      'Authorization',
      'Bearer '+token
    )
    return this.http.get(`https://localhost:7044/api/Addresses/${id}`,{headers:head })
  }
}
