...
export class HttprequestservicesService {
  ...
  //Prodotti
  GetFilteredProduct(prodname: string, page: number, take: number)
  {
    return this.http.get(`https://localhost:7044/api/Products/FilterProducts?pageNumber=${page}&productName=${prodname}&id=${take}`);
  }

  //Prodotti da ottimizzare
  GetTotalFilteredProducts(prodname: string)
  {
    return this.http.get(`https://localhost:7044/api/Products/GetFilterProducts?productName=${prodname}`);
  }
